...
- [Improvement] ``nems-init`` now uses Whiptail for all prompts, providing a more polished overall experience. Laid the CLI groundwork for a headless mode initialization which will tie in to a web-based nems-init process in a future update. Updated to conform with compliance standards set by the new NEMS ULA system, including minimum password length, and a dialogue promoting the use of a role account.
