...
- `mk-livestatus` upgraded from 1.5.0p23 to 2.1.0p37 which is the most current version compatible with Bookworm (which includes gcc++-12 - gcc++-13 won't be available upstream until Trixie). Implemented Sven Nierlein's patched Store.cc (and corresponding logger.c and logger.h) from `naemon-livestatus <https://github.com/naemon/naemon-livestatus>`__ to fix SEGFAULT on modern Nagios.
