...
- `mk-livestatus` upgraded from 1.5.0p23 to 2.2.0p17, which is typically unable to compile on Bookworm (gcc++-13 won't be available upstream until Trixie). Since previous versions of mk-livestatus SEGFAULT Nagios Core on modern implementations, I patched the source to compile with gcc++-12.
