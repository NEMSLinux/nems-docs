...
- [nems-scripts] Added the current global version of NEMS Linux to dynamic file `/var/www/html/inc/ver-available-global.txt` revealing if a newer version is available for other platforms (not just the currently running one).
