...
**NEMS Migrator's ``nems-restore`` will import hosts and assign them the Linux OS even if they are not Linux.**
...
**NEMS Migrator's ``nems-restore`` will remove all example hostgroups from NEMS NConf.**
